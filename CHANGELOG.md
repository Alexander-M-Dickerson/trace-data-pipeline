...
reached, because the published window was pinned in constants. It now runs to whatever
month the Stage-2 panel actually holds; `--sample paper` reproduces the published window
for anyone checking against the printed tables. One switch at the top fans out to each
section's own flag, so running a driver by hand is unchanged.

**Every caption states its own sample.** Titles are untouched. What is new is the
sentence at the end -- "Sample: 2002-09 to 2025-11, T=279", or "T 268-279 by series"
where each series is its own length, or a path count where the exhibit counts
construction paths rather than months. It is derived from the data the exhibit actually
used, never typed, and a test checks that every rendered table carries one. Three
exhibits are exempt because they have no sample by nature -- the two parameter tables and
the definition table -- and the exemption list says why for each.

Before this, one sample was claimed for the whole document while the data appendix in
the same PDF had been computed to 2025-11 and printed a 2025 row.

### Added

- **`stage3/INDEX.md`** -- every table and figure the report prints, mapped to the file
  that produces it, its LaTeX label, its caption title and the sample it states. 44 rows.
  Generated by `tools/build_index.py`, gated by `--check`. The driver comes from each
  result's own manifest, so a rename cannot leave a stale row.
- **Table IA.VIII** (*Signal Definitions and Citations*) is now produced, from
  `stage3/spec/signal_definitions.json` rather than from data -- it is a definition list.
  The spec was reconciled against Stage 2's data dictionary and against the code that
  computes each signal; `stage3/RECONCILIATION_ia08.md` is the full account.
- **A status ledger for Section 5.** One row per grid cell, one status, and every printed
  denominator reads from it -- so Table 6, Table IA.XVIII and Table IA.XIX agree by
...
- **Table IA.VIII's definitions, in six rows**, each settled by the code or by the paper
  contradicting itself. The one that matters: `dcs6`'s missing-spread lookup is a
  +/-1 month search preferring the earlier month, not the +/-2 with "+ favoured" that was
  printed -- wrong in the width and backwards in the tie-break, on an FDR survivor. Also
  `mdc_rat`'s rating scale, and four momentum rows citing the wrong paper by the same
  authors in the same year.
- **`bash run_stage3.sh` reached only step 27 of 40.** A failed exhibit abandoned the
  run, and one check is red by design while an upstream engine defect is open. A failed
  exhibit no longer stops the chain -- the PDF is still produced and the exit code is
  still non-zero. A failed producer still stops it, because everything downstream would
  read missing or stale data.
- **`reports/*.log` carried absolute home-directory paths.** The purge gate globbed
  `data/**/*.json` and `reports/*.tex` and never opened a `.log`. It now covers all of
  `reports/`, and the transcripts are deleted on a successful compile.
- **Table 6's footnote did not add up**: it named one exclusion where there are two.
  Both are printed and a test does the subtraction.
- **`--window full` was passed to a script that has no such flag**, found by a cold run.
  It had always passed because that producer was always skipped. A test now reads each
  driver's own `add_argument` calls and checks every flag the orchestrator would append.
- **`--no-bench`** was declared on 23 drivers and did nothing. It suppresses the
  `timings.jsonl` line, which is what its name says.
- A test asserted that Stage 3's two cluster maps differ on `b_rvol`. Measured: the two
  partitions are identical. It compared cluster NAMES, three of which differ because one
...
- Dead code, each checked for callers first: `mua_engines.run_slow` and its three
  decoders, `clusters.exclude_redundant`, `fastrun.worker_threads`, `paths.cube`,
  `helper_functions.to_percent`, a `return_formation_counts` parameter nothing requests,
  and a filter branch for a type the DUA grid does not produce.
- Internal tracker IDs from user-facing strings. A public reader has no tracker to look
  them up in; each now says what it stands for.
